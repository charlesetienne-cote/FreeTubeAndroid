...
async function findChannelById(id, backendOptions) {
  try {
    if (!(process.env.IS_ELECTRON || process.env.IS_ANDROID) || backendOptions.preference === 'invidious') {
    ...
    if ((process.env.IS_ELECTRON || process.env.IS_ANDROID) && backendOptions.fallback) {
