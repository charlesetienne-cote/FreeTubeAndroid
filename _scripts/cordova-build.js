...
(async function () {
  ...
  if (pathParts[pathParts.length - 1] === pathParts[pathParts.length - 2]) {
    console.log("Directory weirdness detected!");
    // repeating directory (which is indicator that we are in GitHub Actions and should expect weirdness)
    // there is an issue with github actions which causes 
    // __dirname to contain the value /home/user/FreetubeCordova/FreetubeCordova/_scripts
    // instead of the real directory name 
    // /home/user/FreetubeCordova/_scripts
    pathParts.pop()
  ...
  const sourcePackage = JSON.parse((await fsReadFile(path.join(__dirname, '..', 'build', DIST_FOLDER_NAME, 'package.json'))).toString())
