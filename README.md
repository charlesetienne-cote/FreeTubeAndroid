...
### Known Issues:
  ...
  - LocalAPI doesn't work. I don't know exactly why, but I might guess it is CORS. This sometimes causes infinite loops of fallback to invidious (which may fail for any number of reasons) and fallback to local api (which will always fail). It doesn't freeze the application, but when the application gets in this state, the easiest fix is restarting the application (because the invidous server changes on restart)
